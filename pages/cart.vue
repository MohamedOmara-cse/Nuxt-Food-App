<template>
  <main class="container cart">
    <h1>Your Cart</h1>
    <section v-if="!empty" class="emptycart">
      <svg
        height="100px"
        width="100px"
        fill="#000000"
      ...
      <a href="/"><button>Fill it up!</button></a>
    </section>
    <section v-else>
      <table>
        <thead>
        ...
        <tbody>
          ...
          <tr>
            <td colspan="3"></td>
            <td class="total">TOTAL PRICE : {{ orderTotalPrice }} $</td>
          </tr>
        </tbody>
      </table>
    </section>
...
import foodlogo from "@/components/foodlogo.svg";
...
export default {
  ...
  methods: {
    ...
    deleteOrder: function (e) {
      this.$store.commit("deleteOrder", e.target.id);
    },
  },
  computed: {
    ...mapState(["cart"]),
    orderTotalPrice: function () {
      let total = 0;
      for (let order of this.cart) total += order.price * order.amount;
      return total;
    },
    empty: function () {
